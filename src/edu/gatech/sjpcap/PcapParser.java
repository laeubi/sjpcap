...
public class PcapParser {
	...
	private int readBytes(byte[] data) throws IOException {
	...
	private int readGlobalHeader() throws IOException {
	...
	public int openFile(String path) throws IOException {
	...
	private int getTCPHeaderLength(byte[] packet) {
		...
		int dataOffset = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + inTCPHeaderDataOffset;
	...
	private UDPPacket buildUDPPacket(byte[] packet, long timestamp) {
		...
		UDPPacket udpPacket = new UDPPacket(this.buildIPPacket(packet, timestamp));
		int srcPortOffset = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + inUDPHeaderSrcPortOffset;
		udpPacket.src_port = this.convertShort(packet, srcPortOffset);
		int dstPortOffset = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + inUDPHeaderDstPortOffset;
		udpPacket.dst_port = this.convertShort(packet, dstPortOffset);
		int payloadDataStart = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + PcapParser.udpHeaderLength;
		byte[] data = {};
	...
	private TCPPacket buildTCPPacket(byte[] packet, long timestamp) {
		...
		TCPPacket tcpPacket = new TCPPacket(this.buildIPPacket(packet, timestamp));
		int srcPortOffset = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + inTCPHeaderSrcPortOffset;
		tcpPacket.src_port = this.convertShort(packet, srcPortOffset);
		int dstPortOffset = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + inTCPHeaderDstPortOffset;
		tcpPacket.dst_port = this.convertShort(packet, dstPortOffset);
		int payloadDataStart = PcapParser.etherHeaderLength + this.getIPHeaderLength(packet) + this.getTCPHeaderLength(packet);
		byte[] data = {};
		if((packet.length - payloadDataStart) > 0) {
			data = new byte[packet.length - payloadDataStart];
			System.arraycopy(packet, payloadDataStart, data, 0, data.length);
	...
	private PcapPacketHeader buildPcapPacketHeader() throws IOException {
		...
		pcapPacketHeader.timestamp = (this.convertInt(header, inPcapPacketHeaderSecOffset) * 1000) + (this.convertInt(header, inPcapPacketHeaderUSecOffset) / 1000);
	...
	public Packet getPacket() throws IOException {
		...
		if((this.isUDPPacket(packet) && (packet.length < udpMinPacketSize)) || (this.isTCPPacket(packet) && (packet.length < tcpMinPacketSize))) {
			return new Packet();
		}
